...
public class ItemEffectEntropyLimit extends ItemEffectShield
{
	...
	{
		if (strength > 0.0f)
			list.add(String.format("&fReduces received wither-based damage by &b%.0f%%&f", 100.0f * strength));
		else if (strength < 0.0f)
			list.add(String.format("&fIncreases received wither-based damage by &c%.0f%%&f", -100.0f * strength));
	}
}
